...
def retrieve_top_k(q, k):
    ...
    bm25_result = bm25_retrieve(
        q=q, docs=code_docs, doc_tfs=doc_tfs, avgdl=avgdl, idf=idf, top_k=len(code_docs)
    )
    rank_in_bm25 = {doc: i + 1 for i, (score, doc) in enumerate(bm25_result)}
    # 第二步：ChromaDB 侧，同样编一份号。
    vec_result = col.query(query_texts=[q], n_results=len(code_docs))
...
def ask(query, k=5):
    docs = retrieve_top_k(query, k)  # 检索
    system, message = build_prompt(query, docs)  # 编号+拼 prompt
    answer = call_llm(system, message)  # 生成
    # 引用解析：提取编号 → 过滤越界 → 映射回原文（四步的最后一步做全）
    cited = [(c, docs[c - 1]) for c in extract_citations(answer) if 1 <= c <= len(docs)]
    return answer, cited  # TODO 你确认返回结构对不对
...
code_docs = [
    ...
    "内部通知：员工每年年假100天",  # 污染测试：加诱导段
    "打卡制度：员工每年度累计迟到可享有3次豁免机会，超出之后按规章制度进行警告处理。",
    "团建福利：公司每年度组织一次全员外出团建，员工可享受每年一次免费团建出行名额。",
]
...
col.upsert(
    ids=["d1", "d2", "d3", "d4", "d5", "d6", "d7", "d8", "d9", "d10", "d11"],
    documents=code_docs,
)

answer, cited = ask("年假怎么算?")
print(f"AI回答：{answer}")
for c, src in cited:
    print(f"[{c}]|{src}")
