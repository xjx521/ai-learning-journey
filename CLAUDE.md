...
当前阶段：第五阶段（加速计划，已收官） | 当前进度：Day 61 LangGraph 映射 完成（加速阶段提前收官，Day62 整合推迟到 9 月简历项目二阶段） | LeetCode：0题 | 最后更新：2026-08-31
...
| Phase 5 | 8/29-9/1 | 58-61 | Agent 概念 + 手写 ReAct + 真实工具 + LangGraph |
| 9 月收尾 | 9/2 起（开学后） | - | 双项目企业级：Day62 整合（RAG 知识库 Agent）+ 评估体系/Docker/README/架构图/博客 |

> ⚠️ 2026-08-31 决定：**加速计划提前收官于 Day61**（用户看黑马 P6-07~13 觉得 Day62 简历项目二整合内容过于简单）。Day62 任务保留在文件夹里，推迟到 9 月开学后的简历项目二阶段，与 9 月企业级收尾合并做（届时每天 4-5h）。核心概念（Agent + 工具契约 + LangGraph + checkpointer）已在加速阶段全部学完并过 5 关。
...
| 日期 | Day | 完成题数 | 掌握度 | 备注 |
|------|-----|---------|--------|------|
| 08-29 | Day58 | 实验A/B/C+自测5关+初体验 | 良好 | Agent 概念 + 手写 ReAct（Phase 5 第一天，raw 版：不碰 LangGraph 先理解 Agent 本质；概念预习黑马课 P6 Agent智能体 按优先级看 01介绍/04 ReAct框架/02初体验/03流式 4 节，06-13 项目实战分到 Day59-62）：实验A认识 ReAct 协议★(纯文本协议：不用 tools 参数，system 规定 Thought/Action/Action Input/Final Answer 格式；实测 7B 模型单次请求只回 Action 不给 Final Answer——没执行工具+没回传 Observation+7B 协议稳定性有限；没按格式→few-shot 给正确示例有效)、实验B手写最小 ReAct 循环★★★(run_agent：for step in range(max_iterations)→call_llm→含 Final Answer 就 return→re.search 解析 Action/Action Input→TOOLS 字典查函数执行→Observation 拼回 messages 继续→超限强制停止；实测单步2步/多步(天气+跑步)3步；参数怪癖：模型可能传 city=北京 或 JSON→get_weather 里 split("=",1) 剥前缀；工具名解析不到→Observation=f'没有{name}工具'不崩；max_iterations=2 执行完 step1 强制停)、实验C破坏性对照★★(①工具真抛异常→Agent 层 try/except 捕获包装成 Observation 给模型看、不向外抛=生产正确做法，模型拿到错误能自己调整 ②max_iterations=1 强制停止；无限制会 Thought→Action→Observation 死循环烧 token ③FC vs ReAct：FC 解析稳 API 保证但只能等模型发 tool_calls 且依赖模型支持；ReAct 纯文本任何模型能跑但靠提示词约束、小模型易跑歪)。自测5关：能讲(普通 LLM 一次生成无循环/FC 只发工具调用无自动循环/Agent=LLM大脑+工具手脚+循环决策；ReAct 四步 Thought→Action→Observation→Final Answer)、能写(默写 run_agent 骨架，小笔误 max_iteration/messges/f-string 无伤大雅，正式代码全对)、能修(没按格式→查提示词→加 few-shot→换大模型；工具名解析不到→查拼写；循环不停止→查 max_iterations)、能答why(Observation 必须拼回 messages 不能存 Python 变量=大模型无外部变量读取能力、全部上下文靠对话历史；必须限制迭代=防死循环资源耗尽工程兜底)、能扛边界(外部 API 工具最先扛不住=超时限流报错无控制反复重试；上下文随多轮累积 token 暴涨；长任务每轮调模型延迟高；工具变多小模型选错工具→缓解=工具加超时/重试/熔断+异常包装 Observation、max_iterations 合理值+长任务状态压缩/摘要、工具过滤只给相关工具、中间状态校验识别重复调用打断死循环)。✅自评：能讲最吃力(概念理解简单但讲不出，需多复述练习)；自评"run_agent 直接替代成 create_agent"——方向对，Day61 逐行映射 StateGraph 节点/边更精确。文件：exp_a_react_single_step.py+exp_b_react_loop.py+exp_c_react_destruct.py+第一次Agent初体验.py(额外：LangChain create_agent+@tool+ChatOllama 本地算 BMI，stream_mode="values" 看 tool_calls，API 已实测 langchain 1.3.17 存在)。学习笔记《Agent概念与手写ReAct》已补+概念地图 Phase5 第一行回填✅/进度更新 Day59。⚠️待改进：①任务单概念预习标题行被误改成 eeeeeeee 已修复 ②exp_c get_weather raise KeyError 是故意破坏性，生产应工具内 try/except 返回错误提示(exp_b 的 table.get 默认值)或 Agent 层 try/except 双保险 ③初体验脚本 get_height description 笔误"单位厘米2"→description 质量直接影响小模型选工具 ④GBK 中文乱码仍为环境问题非代码错。下一步 Day59 真实工具(raw ReAct + 真实世界交互) |
| 08-30 | Day59 | 实验A/B/C+自测5关 | 良好 | 真实工具（Phase 5 第二天，raw→framework 的"raw 侧补强"）：实验A三只真工具★(get_time 真系统时间/get_weather 调 Open-Meteo 双 API——geocoding 动态查经纬度[替代任务单写死的 CITY_GPS，能查任意城市]+forecast+timeout=10+raise_for_status+try/except 失败包装返回字符串，北京实测 27.3°C 风速 7.9/get_add；⚠️任务单要求 raw run_agent 版没写，三个脚本全用了框架 @tool+create_agent，自评"按@tool实际开发来写"——Day60 补 raw 版)、实验B文件系统工具★★(NOTES_DIR 沙盒+_safe_path 四层防御：strip→正则早退挡`..`/路径分隔符→basename→abspath；save_note 真实落盘 notes/note.txt 文件真写出来了+FileNotFoundError 友好提示；description 写死约束"filename 必须与用户说的完全一致"防模型自行改名/新建；read_note 读回成功但模型下一轮没转述=7B 模型侧问题非工具错)、实验C破坏性对照(⚠️无破坏性代码只是 5 工具合并 demo；"东京查不到"用了首尔 geocoding 能查到失败分支没触发；观察结论填表：网络挂→模型尝试几次自动截止/路径穿越被挡/手写TOOLS靠SYSTEM文本 vs @tool description 自动转 schema)。自测5关：能讲(玩具工具写死无副作用 vs 真实工具联网读写有副作用会慢会挂会不安全；返可读字符串模型才能继续推理；抛异常直接崩 Agent，包装成错误文本给模型它才能决定重试/换工具/告知用户)+铁律"工具内部错误全部消化向外只输出字符串"、能写(默写 get_weather：输入归一化→getcode 查经纬度→requests+timeout→解析 JSON→失败包装，小笔误 ciity/中文逗号/geo 判断 ==→isinstance 正常)、能修(url 路径→网络→工具权限→返回值格式)、能答why(包装成 Observation 不 raise=模型靠 Observation 再思考决定重试或告知用户保证循环继续；Agent 能访问的路径必须限制=沙盒+最小权限，否则提示注入可操纵读写任意文件)、能扛边界(最先撑不住=外部 API 超时限流抖动，其次文件系统安全风险路径穿越，再 token 膨胀、长任务延迟、工具多选错；权限边界三件套=文件沙盒+API超时重试熔断+最大迭代/工具过滤)。✅自评：解析 JSON 和路径安全最卡（re 正则不够好/os 没专门学过但常用）；"工具契约三件套"能用自己话讲清；"手写 run_agent+TOOLS 会被 @tool(description) 和 create_agent 接管"。⚠️待改进：①raw 手写版没写(Day60 补)②实验C无破坏性代码+东京用例失效 ③exp_a:43/exp_c:103 笔误 `.strip('"').strip('"')` 第二个应 `.strip("'")` ④exp_a 未用 import re/client ⑤read_note 模型不转述=7B 模型侧非工具错。下一步 Day60 工具契约复盘+多工具协同 |
| 08-31 | Day60 | 实验A/B/C+自测5关 | 良好 | 工具契约复盘+多工具协同（Phase 5 第三天/raw收尾，压缩版4h，补Day59缺口）：实验A raw快速跑通★(把Day59三只真工具去@tool装回Day58手写run_agent，补Day59只写了@tool框架版没写raw的缺口；对比@tool结构化tool_calls vs raw自由文本Action Input→**@tool解析更稳**；踩坑：get_add(a,b)双参数vs单字符串契约→"3,5"一起传TypeError加法失败+异常handler return打断循环+\S+正则截断——用户在exp_b全改对)、实验B多工具协同★★★核心(**手写ReAct四道缝**总结：①参数怎么传=单字符串契约内部split②Observation从哪来=stop=["Observation:"]物理防模型自编③出错=异常包成Observation不return④终止=Final Answer优先判断否则空转；跑通"查北京天气→记到note.txt"调2次工具get_weather→save_note，note.txt真存22.6°C=Observation拼回messages模型才记得第一步；"刚才记的什么"read_note读出但模型不知是不是刚记的=两次独立run_agent无跨请求记忆stateless边界，用户诊断正确)、实验C破坏性对照(description写糊→7B没选错但纠结跑完20次迭代/8工具没挑花眼；exp_c加5个凑数工具get_poem/stock/news/sqrt/translate)。自测5关：能讲(@tool(description)转schema vs raw TOOLS靠SYSTEM文本；工具契约三件套=名称/描述/参数)、能写(多工具协同循环关键=messages.append assistant回复+Observation user消息，第二次调用才能读上一步结果)、能修(第二次调用忘记→查Observation有没有拼回messages)、能答why(description质量直接影响选对工具=写错即误导)、能扛边界(20工具=先按提问过滤再分组只喂相关工具)。✅自评：最卡改造（写习惯langchain了回到手写run_agent不习惯）；四道缝能用自己话讲；"@tool声明的工具在LangGraph里会变成图的节点函数"预判正确；加第6个工具delete_note设计合理。⚠️待改进：①exp_a的get_add/异常handler/正则三处exp_b已改对②exp_a:43 strip笔误残留Day59③exp_c文件头docstring残留"实验B"④跨请求记忆靠拼历史（Day62项目整合可用checkpointer）。下一步Day61 LangGraph映射(StateGraph节点/边/条件路由，P6-05中间件+P6-12) |
| 09-01 | Day61 | 实验A/B/C+中间件+自测5关 | 良好 | LangGraph映射（Phase 5 第四天/加速阶段收官，把Day58手写run_agent画成图）：实验A认识StateGraph最小图★(State/节点/边/START→END/compile/invoke，count=3跑通+app.stream逐节点看执行顺序n1→n2→n3=图比for循环好调试的直接证据)、实验B把run_agent逐行映射成LangGraph★★核心(映射表全对：for循环→边回环+recursion_limit/call_llm→agent节点/TOOLS执行→tools节点/Final Answer判断→条件路由/messages.append→add_messages reducer；⑤tools节点⑥条件路由⑦tools→agent回边三个空全填对；跑通"北京天气"agent→tools→agent→END+stream观察messages长度1→2涨；"3+5等于几"直接agent→END不经tools=条件路由分支)、实验C破坏性对照★★(checkpointer跨请求记忆=MemorySaver+thread_id=1两轮带记忆答出名字/thread_id=2全新会话无记忆对照成立，解决Day60"刚才记的什么"不记得=State持久化；recursion_limit=10兜底防死循环报RecursionError不停烧token；create_agent内部就是agent节点+条件路由+tools节点这张图=不是黑盒)、middleware中间件预习(P6-05/12：before_agent/after_agent/before_model/after_model/wrap_model_call/wrap_tool_call六个钩子全跑通，跑"惠州天气"按序触发；⚠️黑马课示例3处API笔误已修=handler[request]→handler(request)/tool_call["arg"]→["args"]/monitor_tool补return handler(request)，否则工具节点拿到None报Unsupported message type)。自测5关：能讲(State共享存储messages+节点对应call_llm/TOOLS执行+边固定顺序+条件路由对应if Final Answer判断；手写靠for循环、LangGraph靠条件边回跳节点实现循环)、能写(默写最小StateGraph五步：State→节点函数→add_node→add_edge→compile/invoke)、能修(Expected a dict但节点没return dict且键不是State字段)、能答why(图有可视化/可断点/可持久化/条件路由灵活=比手写循环工程化)、能扛边界(要不要先查库这种分支用条件路由add_conditional_edges；多Agent协作=每个子Agent封装节点靠边和条件路由调度)。✅自评：条件路由最卡(已能自己复述"add_conditional_edges读state判断跳哪个节点")；create_agent不再是黑盒("实际上就是agent-tool-agent-end的步骤而已")；raw→framework链路(手写run_agent→手写LangGraph图→create_agent高层封装)能自己复述三层优缺点。⚠️待改进：①exp_c第二轮7B偶尔答"没提供名字"=模型侧不稳定与Day60"模型不读Observation"同类，非框架bug ②middleware中间件.py黑马示例3处笔误已修 ③概念地图Phase5第3/4行已回填✅。📌重大决定：Day62简历项目二整合推迟到9月(用户看黑马P6-07~13觉得内容过于简单)，加速阶段提前收官，Day62任务单保留当参考，9月与简历项目企业级收尾合并做。学习笔记《LangGraph映射》+《Phase5面试冲刺补充》已补+概念地图Phase5全✅。下一步9月：Day62 RAG知识库Agent+双项目企业级收尾(评估/Docker/README/架构图) |
| 08-29 | Day57 | 实验A/B/C+自测5关 | 良好 | RAG 服务封装（Phase 4 第八天/收尾，简历项目一地基，把 Day56 管道封装成可复用类+补两个概念空白+RAG课P35/P36了解）：实验A封装RAGService类★★(__init__建一次：按persist_dir幂等建/加载Chroma→按retriever_type选vector/bm25/ensemble→拼防幻觉标引用prompt→ChatOllama+LCEL chain，存self；ask四步：retriever.invoke→enumerate编号拼context_with_index→chain.invoke→re提编号过滤越界映射metadata src+原文；返回(answer,[(src_id,原文)])；破坏性对照改配置不改代码：k=2漏请假段(排3-5名)答"没有提及"vs k=5全答出=多答案题保Recall；vector vs ensemble小库都答对无差异=差异要靠更难问题+更大候选池)、实验B文档加载+分块+增量索引★★补概念地图两行(TextLoader加载792字员工手册自动带source metadata→RecursiveCharacterTextSplitter(chunk_size=100,overlap=20)切15块，overlap防边界切断语义→add进库count=23；增量更新先delete ids再add同id→count不涨幂等；新增emp#17→24；删除→23=改配置不整库重建)、实验C整合版破坏性对照+evaluate补全★★(库外"空调坏了找谁修"答"不知道找谁修"=防幻觉兜住；污染库独立chroma_data_day57_pollution加d9"年假100天"→模型引用[1]正式政策+[3]内部通知指出差异需确认=Day55没盲信复现；evaluate把Day56三bug全修：单答案命中即hit不需要recall/多答案elif list分支补上算len(set交集)/len(golden)，5题src id评估集top_k=1 hit0.4 recall0、top_k=3 hit0.4 recall0.5=多答案正确答案在3-4名只查top1拿不全)。自测5关：能讲(svc.ask线上四步+Loader/Splitter属离线预处理对应手写读文档/分片)、能写(默写__init__三件事+ask四步，小笔误as_retriever/Ensembleriever/self.lllm)、能修(k越界→ollama服务→metadata缺src→引用编号越界)、能答why(封装减少冗余提高可读性；upsert/delete保证幂等避免重跑报错=对比Day52重跑from_documents)、能扛边界(10万文档1万次调用最先扛不住=LLM并发其次检索耗时然后库膨胀，单文件只是维护问题)。✅自评：手写→LangChain→服务类映射走完"哦原来这么封装"；换embedding只改svc=RAGService(embed_model=text-embedding-v3)一处不用动管道；概念地图Phase4全✅但自己"其实都不是很熟练"需复习。⚠️待改进(助手review实测复现)：①exp_b ids用set无序→id和chunk可能错配，count验证看不出来，应改list推导式②exp_c evaluate用了污染库svc2的检索器(全局ensemble绑定污染库)严格应指向干净库svc1③exp_c k=2检索器测top_k=3意义不大，evaluate的top_k应≤检索器k④三文件仍无docstring(__main__保护已补)⑤GBK乱码环境问题非代码错。下一步Day58 Agent概念(Phase5第一天) | LangChain 映射 RAG（Phase 4 第七天，raw→framework 核心：逐个问"我手写的函数在 LangChain 里叫什么"+RAG课P10了解）：实验A LangChain最小RAG闭环★(五件套映射全对：裸字符串→Document(metadata存src)/col.query→as_retriever/手写bm25→BM25Retriever挂jieba/手写rrf_fuse→EnsembleRetriever/prompt拼接→ChatPromptTemplate/call_llm→ChatOllama/整条for循环→LCEL管道 `prompt\|llm\|StrOutputParser`；Chroma.from_documents显式ids幂等；as_retriever(search_type="mmr")；4问全答对；k=2漏报销k=5补上=多答案题暴露Recall)、实验B三路检索映射★★(EnsembleRetriever(retrievers=[bm25,vec],weights=[0.5,0.5])内部就是RRF名次融合；**jieba挂preprocess_func是刚需**——注释掉重跑检索明显变差带英文/编号段被顶最前；排序与手写版不一致=LangChain先各自召回再融合机制等价数字不同；编号题R-2024-0015仍答对)、实验C引用映射metadata src id★★(Day55升级版：src存Document.metadata→引用[3]→re提取→过滤越界→映射回src=d4+原文，临时编号只给模型真实id给后端=Day55笔记第6节工程方案落地；交叉验证：库外"空调坏了找谁修"答不知道/污染段d9"年假100天"引用两段指出冲突没盲信/top_k=5引用没飘=Day55三连在LangChain版全复现没跑偏；加分evaluate golden改用src id锚点)。自测5关：能讲(7条映射全对+优缺点：统一接口可插拔/LECL声明编排/自带观测 vs 封装黑盒出bug翻源码/版本升级接口变)、能写(默写LCEL最小chain docs→Chroma.from_documents→as_retriever→prompt→llm→chain\|parser→invoke，小笔误Choram/Ebeddings无伤大雅)、能修(报错看环节→k→ollama服务→prompt变量名)、能答why(换模型/检索器只改组件+生产件齐全Loader解析PDF评估日志+和LangGraph打通)、能扛边界(Chroma两点撑十万文档=persist_directory落盘HNSW索引启动复用+ANN近似检索不遍历全部；BM25Retriever全内存10万段性能差→生产换Elasticsearch)。✅自评：from_documents参数多最卡；jieba坑撞上了体会=没中文分词默认优先英文数字；"框架=把手写步骤变可替换标准件"能复述但强调暗箱+版本更新快。⚠️待改进(助手review实测复现)：①exp_c evaluate加分项半成品三处已全修（eval_set_lc未定义NameError→自造12题评估集含2多答案；set(golden)&len类型错→改set；多答案list分支被跳过→recall恒0.0，修后top_k=3：hit 0.833→0.917、recall 0.0→0.75=11/12两题多答案被计入）②exp_a注释残留生成器print已清③三个文件无docstring/exp_a/b无__main__保护④GBK控制台中文乱码环境问题非代码错。下一步Day57 RAG服务封装(简历项目一整合版)+Phase4收尾复习 |
