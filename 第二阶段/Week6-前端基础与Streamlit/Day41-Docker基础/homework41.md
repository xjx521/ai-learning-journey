...
- 记录结果：答：_____________INFO:     Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)
INFO:     172.17.0.1:58982 - "GET /docs HTTP/1.1" 200 OK
INFO:     172.17.0.1:58982 - "GET /openapi.json HTTP/1.1" 200 OK___________________________________

📝 **任务 5.3**：用 `docker ps` 看容器，用 `docker logs` 看日志，用 `docker stop` 停止。
- 记录你用的命令：______docker ps docker logs docker stop__________________________________________
