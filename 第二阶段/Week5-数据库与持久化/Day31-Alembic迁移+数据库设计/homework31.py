...
# ==================== 参考答案 ====================
# 1.1：Alembic 需要读取 SQLAlchemy 模型类来对比当前数据库状态。如果没有 models.py，
#      Alembic 就不知道你的表应该长什么样，无法生成差异脚本
# 1.2：是的，只能一次。再次执行会报 "Alembic directory already exists" 错误。
#      如果需要重新初始化，必须先删除旧目录再 init
# 1.3：因为 env.py 在一个子目录 alembic/ 里运行时，Python 找不到父目录下的 models.py。
...
# ==================== 参考答案 ====================
# 2.1：第二次生成的是 ALTER TABLE ... ADD COLUMN last_login 和 ADD COLUMN role
#      而第一次是 CREATE TABLE users。ALTER 比 CREATE 简单多了，这就是增量迁移的优势
# 2.2：SQLite 不支持直接修改列类型！Alembic 会生成一个复杂的脚本来重建表（拷贝→删旧→建新→恢复数据）
#      但这容易出问题（比如数据类型不兼容），所以很多 ORM 框架建议你手动处理这类修改
...
# ==================== 参考答案 ====================
# 3.1：在。downgrade 只是执行反向 SQL（如 ALTER TABLE DROP COLUMN），它不会删除表中已有的行数据
#     但如果你 downgrade 了一个包含 CREATE TABLE 的版本（base → 第一步），那整张表都没了，数据自然也没了
# 3.2：如果 downgradle 删除了某列，那这列上的所有数据都会丢失且不可恢复。
...
你的答案：______________________________________________________
...
问题 5.1：加了索引后查询变快了多少倍？
你的答案：__________（一般小表不明显，10000+ 条数据时差距开始显著）

问题 5.2：如果我对 created_at 也建了索引，但查询条件是 `WHERE category = ? ORDER BY created_at DESC`，
...
-- ❌ 不能用索引的前半部分，但可能用到后半部分（SQLite 特殊优化）
