...
你的答案：___看到一个交互式界面 列出了GET/接口__________
...
| URL | 期望结果 | 实际结果 |
|-----|---------|---------|
| /users/123 | 成功，type 为 int | _成功,type为int________ |
| /users/abc | 失败，422 错误 | _失败,422________ |
| /users/3.14 | 失败，422 错误 | _失败,422________ |
| /users/ | 失败，404 错误 | __失败,404错误_______ |

问题 2.1：user_id 被自动转换成了什么类型？
你的答案：__int类型___________

问题 2.2：传入 "abc" 时返回的状态码是多少？错误信息是什么？
你的答案：___422 错误信息是 Input should be a valid integer, unable to parse string as an integer 输入的user_id应该是整型__________

💡 破坏性实验：
  把 user_id: int 改成 user_id: str，再用 /users/123 测试。
  观察：type 变成了什么？ 字符串类型的123
"""
...
| /items?page=2&size=20 | __{"page":2,"size":20}_________________ |
| /items?keyword=python | _________{"page":1,"size":10,"keyword":python}__________ |
| /items?page=abc | ____422错误（不是整型）_______________ |
| /items?size=5&keyword=fastapi&page=3 | _{"page":3,"size":5,"keyword":fastapi}__________________ |

问题 3.1：哪些参数是必填的？哪些是可选的？
你的答案：____都是可选的都有默认值_________

问题 3.2：keyword 不传时，返回值中有 keyword 字段吗？
你的答案：___没有keyword字段（if keyword 条件不满足）__________

💡 破坏性实验：
  把 keyword: Optional[str] = None 改成 keyword: str（去掉默认值）
  再用 /items 测试（不传 keyword），会发生什么？422错误 缺少必填参数Field required
"""
...
你的答案：___user_id是路径参数_category,limit是查询参数_________

问题 4.2：访问 /users/5/items?category=books&limit=3 的返回值是什么？
你的答案：___{"user_id":5,"category":books,"limit":3}__________

问题 4.3：访问 /users/items（没有 user_id）会怎样？
你的答案：__404报错 /users/items不存在应该为/users/{user_id}/items___________

💡 破坏性实验：
  把参数顺序调换：def get_user_items(category: str, user_id: int, limit: int = 10)
  user_id 还是路径参数吗？（提示：看它是否在路径 {user_id} 中）是路径参数 只要路径没有的才是查询参数
"""
...
测试：
  /sort/asc → __{"order":"asc","message":"按 asc 排序}_______
  /sort/desc → __{"order":"desc","message":"按 desc 排序}_______
  /sort/random → __报错422 值不在枚举选项中_______

问题 5.1：传入 "random" 时返回了什么错误信息？
你的答案：_"Input should be 'asc' or 'desc'"____________
"""
...
你的答案：___201___Created_______

问题 6.2：DELETE 返回 204 时，响应体有内容吗？
你的答案：___没有内容__________
"""
...
问题 7.1：tags 的作用是什么？
你的答案：__接口分组标题___________

问题 7.2：docstring（三引号注释）在 /docs 中显示在哪里？
你的答案：____在详细描述区域相当于description_________
"""
